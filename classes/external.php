...
class mod_pokcertificate_external extends external_api {
     ...
     */
    public static function incompletestudentprofile_view(
        $options,
        $dataoptions,
        $offset = 0,
        $limit = 0,
        ...
        $params = self::validate_parameters(
            self::incompletestudentprofile_view_parameters(),
            [
                'options' => $options,
     ...
     * Returns description of method result value.
     */
    public static function  incompletestudentprofile_view_returns() {
        return new external_single_structure([
            'options' => new external_value(PARAM_RAW, 'The paging data for the service'),
            'dataoptions' => new external_value(PARAM_RAW, 'The data for the service'),
            'filterdata' => new external_value(PARAM_RAW, 'The data for the service'),
            ...
            'records' => new external_multiple_structure(
                new external_single_structure(
                    array(
                        ...
                        'language' => new external_value(PARAM_RAW, 'childs in custom_category'),
                    )
                )
            )
        ]);
    ...
    public static function courseparticipants_view_parameters() {
        return new external_function_parameters([
            'options' => new external_value(PARAM_RAW, 'The paging data for the service'),
            'dataoptions' => new external_value(PARAM_RAW, 'The data for the service'),
            'offset' => new external_value(PARAM_INT, 'Number of items to skip from the begging of the result set', VALUE_DEFAULT, 0),
     ...
     * @return array The list of users and total users count.
     */
    public static function courseparticipants_view(
        $options,
        $dataoptions,
        $offset = 0,
        $limit = 0,
        ...
        // Parameter validation.
        $params = self::validate_parameters(
            self::courseparticipants_view_parameters(),
            [
                'options' => $options,
                'dataoptions' => $dataoptions,
                'offset' => $offset,
     ...
     * Returns description of method result value.
     */
    public static function  courseparticipants_view_returns() {
        return new external_single_structure([
            'options' => new external_value(PARAM_RAW, 'The paging data for the service'),
            'dataoptions' => new external_value(PARAM_RAW, 'The data for the service'),
            'filterdata' => new external_value(PARAM_RAW, 'The data for the service'),
